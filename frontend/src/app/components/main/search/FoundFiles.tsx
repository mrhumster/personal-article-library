...
export const FoundFiles = ({item, query}:{item: FoundFileItemIFace , query: string}) => {
  ...
  const label = <div>
    ...
    {item.highlight && item.highlight['attachment.content'] && item.highlight['attachment.content'].map((highlight: string, key: number) => {
    ...
    <div className={'text-xs text-zinc-400'}>{item._score}</div>
  </div>
