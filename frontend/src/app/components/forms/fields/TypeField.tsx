...
export const TypeField = () => {
  const value = useSelector((state: RootState) => state.articles.new_article?.additional_information?.type)
  const errors = useSelector((state: RootState) => state.ui.leftSideBar.formErrors['additional_information.type'])
  const dispatch = useDispatch()

  const validate = (value: string | null | undefined) => {
    const fieldName = 'additional_information.type'
  ...
  const handleChange = (value: string | null) => {
    validate(value)
    dispatch(setNewArticleAdditionalInformation({type: value}))
  }
