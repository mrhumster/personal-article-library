...
export const SourceField = () => {
  const value = useSelector((state: RootState) => state.articles.new_article?.additional_information?.source)
  const errors = useSelector((state: RootState) => state.ui.leftSideBar.formErrors['additional_information.source'])
  const dispatch = useDispatch()

  const validate = (value: string | null | undefined) => {
    const fieldName = 'additional_information.source'
  ...
  const handleChange = (value: string | null) => {
    validate(value)
    dispatch(setNewArticleAdditionalInformation({source: value}))
  }
